...
function barsHandler() {
    ...
    this.getAttendees = function (req, res) {
        ...
        Pub.findOne({ 'id': barId }).exec(function (err, pub) {
            if (err) throw err;
            if (pub) {
    ...
    this.attendBar = function (req, res) {
        ...
        var nameMatch = function nameMatch(attendee) {
            return attendee.username == username;
        };
        var pubMatch = function pubMatch(pub) {
            return pub.id == barId;
        };
        Pub.findOne({ 'id': barId }).exec(function (err, pub) {
            if (err) throw err;
        ...
        User.findOne({ 'github.username': username }).exec(function (err, attendee) {
            if (err) throw err;
            if (attendee) {
                var filteredPubs = dateFilter(attendee.attending);
                var barIdFound = false;
                for (var i in filteredPubs) {
...
//# sourceMappingURL=barsHandler.server.js.map
